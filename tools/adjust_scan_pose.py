...
def save_to_config(j1: float, j2: float) -> bool:
    """เขียนค่า J1/J2 ใหม่ลงบล็อก SCAN_POSE ใน config.py (ไม่แตะส่วนอื่น)"""
...
saved_j1, saved_j2 = float(SCAN_POSE['J1']), float(SCAN_POSE['J2'])
j1, j2 = saved_j1, saved_j2
step = 1.0

print('=' * 64)
print('ปรับท่าสแกน — ดูภาพสดที่ http://<TAILSCALE_IP>:8081/wrist')
print()
print('  a / d   J1 ลด / เพิ่ม   (J1 น้อยลง = ล้อเลื่อนไปทางขวาในภาพ)')
print('  w / x   J2 ลด / เพิ่ม   (J2 น้อยลง = กล้องเงยขึ้น ล้อเลื่อนลงในภาพ)')
...
def show():
    flag = '' if (j1, j2) == (saved_j1, saved_j2) else '  *ยังไม่บันทึก*'
    sys.stdout.write(f'\r  J1={j1:6.1f}°   J2={j2:6.1f}°   ก้าว {step:.0f}°{flag}        ')
    sys.stdout.flush()
...
while True:
    ...
    if k == 'a':
        ...
        j1, j2 = saved_j1, saved_j2
        print(f'\n  คืนค่าเดิม J1={j1:.0f} J2={j2:.0f}')
    elif k == 'p':
        if save_to_config(j1, j2):
            saved_j1, saved_j2 = j1, j2
            print(f'\n  ✓ บันทึกลง config.py แล้ว — SCAN_POSE J1={j1:.1f} J2={j2:.1f}')
            print('    (อย่าลืม git commit)')
        show()
        continue
    elif k == 'q':
        if (j1, j2) != (saved_j1, saved_j2):
            print(f'\n  ออกโดยไม่บันทึก — config.py ยังเป็น J1={saved_j1:.0f} J2={saved_j2:.0f}')
