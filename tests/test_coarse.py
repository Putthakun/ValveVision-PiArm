...
def test_จุดรวมสายตาอยู่ใต้ดุมตรงๆ_ได้ผลเท่ากับแนวตั้งของภาพ():
    clock = _pixel_angle_to_clock(hub_xy=(640, 400), valve_xy=(640, 600), vp_xy=(640, 5000))
    assert abs(clock - 6) < 0.01


def test_6_นาฬิกาจริงที่ท่า_A_ต้องอ่านได้_6_เมื่อใช้จุดรวมสายตา():
    """ค่าจริง 2026-09-13: ตั้ง 6 นาฬิกา (ยืนยันด้วยตลับเมตรห้อยเป็นเส้นดิ่ง)
    สูตรเดิมที่ถือว่าลงล่างในภาพ = ดิ่งจริง อ่านได้ 5.62 เพราะกล้องก้มลงมองล้อ
    """
    hub, valve = (483, 330), (527, 550)
    assert abs(_pixel_angle_to_clock(hub, valve) - 5.62) < 0.05          # อาการเดิม
    clock = _pixel_angle_to_clock(hub, valve, vp_xy=(717.5, 1579.2))
    assert abs(clock - 6) < 0.15


def test_12_นาฬิกาจริงที่ท่า_B_ต้องอ่านได้ใกล้_12_เมื่อใช้จุดรวมสายตา():
    """ค่าจริง 2026-09-13 (รอบ 2 ที่ 12 นาฬิกา): สูตรเดิมอ่านได้ 11.68"""
    clock = _pixel_angle_to_clock((486, 340), (435, 36), vp_xy=(830.6, 3230.8))
    assert min(abs(clock - 12), clock) < 0.15
