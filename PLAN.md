...
**กล้องตัวเดียว** — Pi Camera Module 3 **Wide** (`imx708_wide`) ติดบนแขนหลัง gripper
เข้าถึงผ่าน `picamera2` · 1280×720 · โฟกัสใกล้สุด ~5 ซม.

| ใช้ในเฟส | แขนอยู่ไหน | เห็นอะไร |
|---|---|---|
| หยาบ | ท่าสแกน (`SCAN_POSE`) | ล้อทั้งวง — หามุมนาฬิกาของจุ๊บ |
| ละเอียด | เข้าใกล้จุ๊บ | จุ๊บเต็มเฟรม — ความแม่นทั้งหมดอยู่ที่นี่ |

**กล้องภาพรวม (Logitech Brio ตรึงกับที่) ถูกตัดออกแล้ว (2026-08)** — เหตุผลและหลักฐานใน [DESIGN.md](./DESIGN.md) หัวข้อ 6
...
| ภาพเบลอ | ถ่ายระหว่างแขนกำลังขยับด้วย | สคริปต์ (ดูหมายเหตุ) |
...
ภาพจากกล้องเดียว แต่มี **2 มาตราส่วน** ที่ต้องเก็บให้ครบทั้งคู่:

```
ต่อ 1 รอบ (คนตั้งไฟ + ขยับล้อ ~30 วิ):
  ท่าสแกน      →   6 ใบ   (A 3 + B 3 · สั่งท่าเพี้ยนเล็กน้อยกันภาพซ้ำ)
  เข้าใกล้      →  15–21 ใบ  (กวาดหลายระยะและหลาย pitch เท่าที่เอื้อมถึง)
  ภาพลบ        →   1–3 ใบ   (เล็งไปตำแหน่งนาฬิกาที่ไม่มีจุ๊บ)

12 ตำแหน่งนาฬิกา × 4 สภาพแสง = 48 รอบ  →  รวม 1,260 ใบ
  (ต่อรอบได้ 23–30 ใบ ไม่เท่ากันเพราะแต่ละตำแหน่งนาฬิกาเอื้อมถึงไม่เท่ากัน
   เช่น 6 นาฬิกาแขนต้องเหยียดลงต่ำกว่าฐาน จึงกวาดได้น้อยกว่า)

รอบที่ 2 — บันทึกจากการรันจริง
  15 ครั้ง × ~30 เฟรม = ~450 ใบ  ให้โมเดลรอบแรกทำนายกรอบล่วงหน้าแล้วไล่แก้
```

> **ตัวเลข 1,632 ใบที่เคยวางไว้ทำไม่ได้จริง** — คำนวณจากสมมติว่ากวาดได้ 25 ท่าทุกตำแหน่ง
> แต่พอบังคับให้ *กล้องต้องเล็งไปที่จุ๊บจริง* (ไม่ใช่แค่ปลายแขนอยู่ใกล้) ท่าที่ใช้ได้ลดลง
> 1,260 ใบยังเกินพอสำหรับ detection คลาสเดียวบนล้อวงเดียว

**ทำไมภาพที่ท่าสแกนต้องเก็บน้อยกว่า:** ที่ท่าสแกนแขนอยู่นิ่งที่เดิม ถ่าย 25 ใบก็ได้ภาพ
แทบเหมือนกัน 25 ใบ ไม่ได้ความหลากหลายเพิ่ม **แต่ทำให้ตัวเลข mAP หลอกตัวเอง**
เพราะภาพซ้ำกระจายไปทั้ง train และ val

**เพดานของภาพที่ท่าสแกน:** ความหลากหลายถูกจำกัดที่ `ตำแหน่งนาฬิกา × สภาพแสง` เท่านั้น
และนี่คือมาตราส่วนที่ตรวจจับ**ยากกว่า** (จุ๊บ ~20–56 px เทียบกับ ~90 px ตอนเข้าใกล้)
จึงเพิ่มความหลากหลายด้วยวิธีอื่นแทนการถ่ายเพิ่ม: ครบ 12 ตำแหน่งรอบวง และขยับล้อทุกรอบ

> ⚠️ **ภาพเบลอเกิดขึ้นจริงได้น้อยกว่าที่คิด** — วัดแล้วภาพที่ถ่ายระหว่างแขนขยับ
> ได้ความคม 157 เทียบกับ 219 ของภาพนิ่ง ต่างกันนิดเดียว เพราะเราจำกัดความเร็วแขนไว้
> 60°/วิ เพื่อความปลอดภัย บวกกับแสงพอที่ shutter จะเร็ว
> **ปล่อยไว้แบบนี้ถูกแล้ว** — ภาพพวกนี้สะท้อนสิ่งที่ระบบเจอจริง (ตอนใช้งานแขนก็ช้าเท่ากัน)
> การทำให้เบลอเทียมคือการเทรนด้วยภาพที่ไม่มีวันเกิดขึ้น
> ไฟล์พวกนี้ตั้งชื่อลงท้าย `_nearblur` เพื่อให้แยกออกตอน label
...
### นิยามกรอบ label — ต้องเหมือนกันทุกภาพทั้งสองมาตราส่วน
...
ภาพที่ท่าสแกนมีรูระบายอากาศและน็อตล้ออยู่ในเฟรมทุกใบอยู่แล้ว **จึงเป็นภาพลบในตัวเอง**
...
### การครอปภาพที่ท่าสแกนก่อนเข้าโมเดล

**ใช้โมเดลตัวเดียว** แต่ทางเข้าของภาพที่ท่าสแกนต้องผ่านครอปคงที่ก่อน
...
**ภาพที่ท่าสแกนของรอบเดียวกันแทบเหมือนกัน** จึงยิ่งห้ามสุ่มแบ่ง —
...
สั่งแขนไปพิกัดเดิม 20 ครั้ง กลับท่าสแกนระหว่างครั้ง ถ่ายภาพที่ท่าสแกนทุกครั้ง วัดว่าภาพเลื่อนกี่พิกเซล
(ตัวเลขนี้สำคัญกว่าเดิม เพราะเฟสหยาบพึ่งพาความซ้ำของท่าสแกนโดยตรง)
...
- [x] **Step 3: เช็คว่ากล้องที่มือโฟกัสที่ 13 ซม. ได้ — ทำก่อนเก็บจริง**
...
- [x] **Step 4: เขียน `tools/collect_dataset.py`**
...
- [x] **Step 5: ทดลองรอบเดียว แล้วเปิดดูภาพทุกใบ**

```bash
ssh pi@<host> 'cd ~/ValveVision-PiArm && python tools/collect_dataset.py --clock 6 --light room'
```

**ต้องดูภาพจริงก่อนเก็บอีก 47 รอบ** ถ้ากรอบท่ากวาดผิด จะเสียเวลาทั้ง 48 รอบ
เกณฑ์: ภาพจากกล้องที่มือ **≥ 70%** ต้องเห็นจุ๊บ (สูงกว่าแผนเดิมเพราะตอนนี้เล็งจากเรขาคณิต ไม่ได้กวาดมั่ว)

- [x] **Step 6: เก็บครบ 48 รอบ**

12 ตำแหน่งนาฬิกา (1–12 ทุกชั่วโมงรอบวง) × 4 สภาพแสง (`day`, `room`, `dim`, `torch`)
...
- [x] **Step 8: commit สคริปต์ (ไม่ commit ภาพ)**
...
- [x] **Step 1: เลือก ~180 ภาพให้คละกัน**

จาก 48 รอบ เอารอบละ ~4 ภาพ **คละทั้ง 2 มาตราส่วน ทั้งภาพชัดและภาพเบลอ ทั้ง 5 ระยะ**

⚠️ **ต้องมีภาพจากท่าสแกนอย่างน้อย 1 ใบต่อรอบ (48 ใบ)** — มาตราส่วนนี้มีความหลากหลายจริง
แค่ 36 แบบ และเป็นด้านที่ตรวจจับยากกว่า ถ้าสุ่มเลือกเฉยๆ จะได้ภาพรวมน้อยเกินไป
(ในภาพดิบ ภาพท่าสแกนมีสัดส่วนแค่ 288 จาก 1,632 ใบ)

รวมภาพลบของกล้องที่มือ ~30 ใบเข้าไปด้วย — label เป็นไฟล์ว่าง

- [x] **Step 2: Label ด้วย Roboflow หรือ labelImg**

class เดียว: `valve`
**กรอบต้องครอบ "ก้านจุ๊บทั้งอัน" ตั้งแต่จุดที่โผล่พ้นขอบล้อถึงปลายสุด** — นิยามนี้ต้องเหมือนกันทุกภาพ ไม่งั้นโมเดลจะสับสน

- [x] **Step 3: จัด train/val ตามรอบ**
...
- [x] **Step 5: export เป็น ONNX แล้วดึงลง Pi**
...
> ⚠️ **ค้างไว้จาก Task 7 (2026-08)** — เสร็จแล้วแต่ยังไม่สะอาด
> - **split ที่ใช้เทรนโมเดลตัวนี้รั่ว** (Roboflow สุ่มแบ่งรายภาพ ไม่ได้แบ่งตามรอบ)
>   ตัวเลข recall 0.947 จึงสวยเกินจริง **ห้ามเอาลงเล่ม** ต้องเทรนใหม่ด้วย
>   `tools/train_colab.ipynb` ซึ่งแบ่งตามรอบให้ถูกต้องแล้ว
> - **ภาพลบ 378 ใบยังไม่เข้า dataset** จึงยังวัดเกณฑ์ false positive ไม่ได้
> - ตำแหน่ง 10 กับ 14 มีภาพที่จุ๊บไม่อยู่ในเฟรม (เล็งพลาดตอนเก็บ ไม่ใช่โมเดลพลาด)
>   ถ้าเทรนรอบหน้าควรคัดออกหรือใช้เป็นภาพลบ
...
**✅ ผ่านแล้ว (2026-09-05):** ทดสอบจริงด้วย `tools/test_coarse_live.py` ครบทุกตำแหน่ง
1-14 นาฬิกา เห็นวาล์วชัดเจนทุกตำแหน่ง (เกินเกณฑ์ ≥8/10) หลังแก้ 3 ปัญหาที่เจอ
ระหว่างทาง: (1) ทิศซ้าย-ขวาของ `_pixel_angle_to_clock` กลับด้าน, (2) เกณฑ์
circularity ของ `_find_hub` เข้มไปตอนดุมโดนขอบเฟรมตัด, (3) `SCAN_POSE_UPPER`
เงยไม่พอ/คลาย J3 พร้อม J2 ทำให้ servo สตอล — ดู git log ของ `coarse.py` และ
`config.py` ช่วง 2026-09-02 ถึง 09-05 สำหรับรายละเอียด
...
- [x] **Step 1: เขียน test ด้วยกล้องปลอมที่จำลองการลู่เข้า**
...
- [x] **Step 5: ทดสอบบนของจริง ดูว่า error ลดลงทุกรอบ**

```bash
python3 -m fine --debug
```

**✅ ผ่านแล้ว (2026-09-12):** ทดสอบจริงที่ 9 นาฬิกา **10/10 ครั้ง converged=True**
(เกณฑ์ผ่าน ≥8/10) error สุดท้ายอยู่ในช่วง 3.4-23.5px ใช้ 2-3 รอบต่อครั้ง
ระหว่างทางเจอและแก้ 4 ปัญหาใหญ่ก่อนจะผ่าน:
1. สูตร nudge ใน `fine.py` ผิดเครื่องหมาย (`d = err×scale` ต้องเป็น
   `d = -err×scale`) — ทำให้จุ๊บวิ่งหนีเป้าแทนที่จะเข้าหา
2. กล้องที่มือเอียงเทียบแกน gripper ~25-30° ทำให้ error เริ่มต้นใหญ่เกินไป
   เสมอ — แก้ด้วยปรับมุมกล้องจริง (`tools/hold_at_valve.py`) แล้วเปลี่ยนเป้า
   เป็นจุดคงที่ที่วัดได้ (`aim_from: calibrated`) แทนหาปลาย gripper สด
3. `pixel_scale.json` วัดไว้แค่ pitch เดียว ทั้งที่แต่ละตำแหน่งนาฬิกาได้ pitch
   ไม่เท่ากัน — เพิ่มระบบเก็บหลาย pitch (`_load_scale_for_pitch`)
4. ตั้งเกณฑ์เข้าเป้า (px_thresh) แม่นกว่าที่แขนทำซ้ำได้จริงทางกายภาพ (วัดได้
   18px ด้วย `tools/measure_repeatability.py`) ทำให้ไล่ตามสัญญาณรบกวนแทน
   ที่จะหยุด — ปรับ threshold ให้สมจริง + หยุดเมื่อไม่ดีขึ้น + กลับท่าที่ดีที่สุด

ดู git log ของ `fine.py`/`coarse.py`/`pixel_scale.json` ช่วง 2026-09-06 ถึง
09-12 สำหรับรายละเอียดทุกจุด
...
- [x] **Step 1: เขียน test ของ state machine ด้วย `ReplayCamera`**
...
- [x] **Step 3: เขียน `run.py`**

```
สถานะ SCAN     → coarse_locate() สำเร็จ? → APPROACH : SCAN
สถานะ APPROACH → fine_align() converged? → TOUCH : RECOVER
สถานะ TOUCH    → เดินหน้าตามแกนอีก TOUCH_PUSH_MM = 5 มม.
                 ★ ถอยทันที ไม่ค้าง ★  → RECOVER
                 (★ แก้จาก "12 ซม." เดิม 2026-09-12 — ตัวเลขนั้นมาจากตอนที่
                  coarse ยังถอยแขนไว้ 12 ซม. ซึ่งถอดออกแล้วเพราะทำให้หลาย
                  ตำแหน่งเอื้อมไม่ถึง ตอนนี้ coarse พาไปถึงระยะสัมผัสเลย
                  ถ้าดัน 12 ซม.ต่อจะพุ่งทะลุเข้าล้อ ยึดกฎ DESIGN.md หัวข้อ 7
                  "ห้ามสั่งเกินตำแหน่งที่ตรวจจับได้เกิน ~5 มม." แทน)
สถานะ RECOVER  → arm.retreat() → arm.go_scan_pose() → SCAN
```
ดัก `KeyboardInterrupt` ให้ `retreat()` ก่อนออกเสมอ — **ห้ามทิ้งแขนค้างอยู่ในซอกล้อ**

- [x] **Step 4: รัน test ให้ผ่าน** (36 เทสผ่านทั้งหมด)

- [x] **Step 5: รันจริงครบวงจร 5 ครั้ง**

> **ผล (2026-09-13, ล้อที่ 12 นาฬิกา): ผ่านเกณฑ์ 5/5 แต่แตะได้ 0/5**
> ไม่ crash ไม่ค้าง และกลับท่าสแกนครบทุกรอบ แต่ยังไม่ได้แตะเลย
> - เฟสหยาบบอกตำแหน่งถูกทุกรอบ (clock ≈ 11.7–11.8)
> - เฟสละเอียดต้องยกแขนขึ้น (err_y +143 ถึง +264 px) แล้วชนขอบระยะเอื้อม (`[IK] นอก reach > 245`)
>   → 12 นาฬิกาเป็นจุดสูงสุดของวง อยู่ขอบพื้นที่ทำงานพอดี ระบบจึงไม่ยื่นไปแตะตามกฎข้อ 2
> - ⚠️ ท่าสแกนไม่นิ่ง: ดุมล้อในภาพท่า B อยู่ที่ y=507 → 342 → 340 → 324 → 324
>   รอบแรกแขนเคยติดฐานมาก่อน ต้องตามดูต่อว่านิ่งแล้วจริงไหม (กระทบ D1 ใน Task 13)
> - ยังไม่ได้พิสูจน์ว่า "แตะได้จริง" ครบวงจร → ควรลองที่ตำแหน่งกลางพื้นที่ทำงาน (เช่น 6 นาฬิกา ที่ Task 10 ผ่าน 10/10)
>
> ตั้ง OS ใหม่ (Python 3.13, venv แบบ `--system-site-packages`) · ต้องล็อก `adafruit-circuitpython-register<1.13`
>
> **ตามต่อที่ 6 นาฬิกา (2026-09-13):**
> - ✅ แก้เฟสหยาบอ่านมุมเพี้ยน ~10° — สาเหตุคือกล้องที่ท่าสแกนก้มลง เส้นดิ่งในภาพลู่เข้าหา
>   จุดรวมสายตา (vanishing point) ไม่ได้ตั้งตรง วัดด้วยตลับเมตรห้อยเป็นเส้นดิ่ง
>   → `tools/measure_vertical_vp.py` + `scan_vp.json` · อ่านได้ 5.94–5.96 แล้ว (เดิม 5.62)
> - ❌ เฟสละเอียดที่ pitch +40° วิ่งหนีเป้า: `pixel_scale.json` entry "45" มี `mm_per_px_y=+0.5`
>   (ใส่ด้วยมือ ก่อนปรับมุมกล้อง 09-12) แต่ของจริงเป็นลบ ~−0.16 ถึง −0.24
> - ❌ วัดใหม่ด้วย `measure_pixel_scale.py --clock 6 --pitch 50` ไม่ผ่าน 4 ครั้ง: รอบที่ดีได้ลบตรงกัน
>   แต่บางรอบ dv เล็กผิดปกติ (+7, −11, −19, +32 px) เครื่องหมายสลับ → สงสัยระยะคลอนเฟือง (backlash)
>   ของ J2/J3 กลืนการขยับ z ช่วงสั้น — ยังไม่ได้แก้ `pixel_scale.json`
> - ❌ ลองเข้าหาทุกจุดจากด้านบน (กันระยะคลอน) → แย่ลง: สั่งลด z แต่ภาพเลื่อนขึ้น 4/5 รอบ แกน x กระจาย 40%
>   (เดิม 2%) ถอยโค้ดส่วนนี้กลับแล้ว · เจ้าของสังเกตว่า **J2 แบกแขนไม่ค่อยไหว**
>   → ⏸️ **ขวางอยู่: ตรวจ J2 ทางกลไก/ไฟเลี้ยงก่อน (TODO.md A5)** ค่าคาลิเบรตใดๆ เชื่อไม่ได้จนกว่าแขนจะไปถึงท่าที่สั่ง
> - แก้บั๊ก `measure_pixel_scale.py` เขียน `aim_from="gripper_visible"` ทับเป้าที่จูนไว้ → ยกค่า aim จาก entry เดิม
> - ตรวจ J2 ผ่าน `arm.py` (ท่า 6 นาฬิกา pitch 50 ห่าง 150 มม.): **ค้าง 30 วิ นิ่ง** (เลื่อน ≤5 px)
>   แต่ **ไปท่าเดิมซ้ำ 5 รอบ จุ๊บตกห่างกัน y 50 px / x 22 px** → ปัญหาคือกลับไปท่าเดิมไม่ตรง ไม่ใช่ทรุด
>   ก้าว z 10–25 มม. ให้สัญญาณแค่ 60–80 px ใกล้ความคลาดเคลื่อน วัดจึงสลับเครื่องหมาย
>   (ลด z 25 มม. ได้ dv +12 และ −60 ในรอบติดกัน) · ⚠️ `tools/probe_j2_stall.py` สั่ง servo ตรง
>   ไม่ผ่าน arm.py (ผิดกฎข้อ 4) และมุม J2 39–48° นอก LIMITS — ห้ามรัน
> - ทดสอบลอยหน้าจุ๊บ (สั่งห่าง 30 มม.): สูง-ต่ำ/ซ้าย-ขวาตรง แต่ห่างจริง 60–70 มม. และสั่งยื่นเพิ่มแล้ว
>   ปลายตกลงแทนเดินหน้า → เจ้าของยืนยัน **ฐานโยก + J2 ยวบ ตอนแขนยื่นไปข้างหน้า**
>
> **🔀 เปลี่ยนแผน (2026-09-13 เจ้าของตัดสินใจ): ทำ end-to-end ก่อน ยังไม่สนความแม่น**
> เปิด Pi → หาจุ๊บ → ยื่นไปใกล้ที่สุดทุกตำแหน่งนาฬิกา → ค้าง 5 วิ → กลับท่าสแกน วนไปเรื่อยๆ
> - `python run.py` = โหมดวน (`hover_once`) ข้ามเฟสละเอียด · `--once` = state machine เดิม
> - **ยกเว้นกฎข้อ 2 ตามที่เจ้าของสั่ง:** `Arm.nearest_reachable()` หดเป้าเข้าหาไหล่จนเอื้อมถึง
>   แต่ `solve_ik` ยังคืน None และระยะที่ขาดถูกพิมพ์ทุกรอบ (ในทางคำนวณถึงครบ 12 ตำแหน่งที่ standoff 10 มม.)
> - `setup/valvevision.service` + `setup/install_service.sh` ให้เริ่มเองตอนบูต
> - J1 มีระยะคลอน ปลายเยื้องขวาแบบสุ่ม → เข้าหาเป้าจากซ้ายเสมอ + หมุนช่วงท้ายช้าๆ ทีละ 0.5° (กันสะบัด)
>   แล้วจูน `ZERO_OFFSET['J1']` = 4.5 ด้วยกล้อง (จุ๊บตรง aim_x ที่ 12 นาฬิกา) · เจ้าของเช็คทุกตำแหน่ง: คลาดยอมรับได้
> - งานที่ค้าง: ยึดฐาน/เสริม J2 (เจ้าของ) → แล้วค่อยกลับมาทำเฟสละเอียดและ Task 12–13
